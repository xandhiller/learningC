...
int savePlayerName (player_t p, FILE * filename);
...
int main (void)
{
  ...
  printf("\n");
  savePlayerName(playerA, filename);
  savePlayerName(playerB, filename);

  filename = fopen ("playerdatabse.txt", "r");
  fscanf(filename, "%s", &testprint);
  fclose(filename);
...
int savePlayerName (player_t p, FILE * filename)
{
  ...
  filename = fopen ("playerdatabase.txt","r");
  if (filename == NULL)
  {
    printf("File error\n");
    return 1;
  ...
  filename = fopen ("playerdatabase.txt","w");
  if (filename == NULL)
  {
    printf("File error\n");
    return 1;
  }
  fprintf(filename, "%s %c\n", contents, p.letter);
  fclose (filename);
  return 0;
}
